//*****************************************************************************
//
// touch.c - Touch screen driver for the Intelligent Display Module.
//
...
//*****************************************************************************
//
// This variable indicates whether we are using a display which reverses the
// direction of the long axis.  The Formike KWH028Q02-F04 display, for example,
// reverses the sense of the long axis relative to the previous version of the
// hardware, KWH028Q02-F03.  The driver supports dynamic selection to allow
// a single binary to run on boards with either display type.
//
//*****************************************************************************
static tBoolean g_bReverseLongAxis = false;
    ...
    switch(g_ulTSState)
    {
        ...
        case TS_STATE_READ_X:
        {
            ...
            HWREG(TS_Y_BASE + GPIO_O_DATA + ((TS_YP_PIN | TS_YN_PIN) << 2)) =
                (g_bReverseLongAxis ? TS_YN_PIN : TS_YP_PIN);

            //
            // Configure the sample sequence to capture the X axis value.
            //
            HWREG(ADC0_BASE + ADC_O_SSMUX3) = ADC_CTL_CH4;
        ...
        case TS_STATE_INIT:
        {
            ...
            //
            // Drive the positive side of the X axis touch layer with VDD and
            // the negative side with GND.  Also, drive both sides of the Y
            // axis layer with GND to discharge any residual voltage (so that
            // a no-touch condition can be properly detected).
            //
            HWREG(TS_X_BASE + GPIO_O_DATA + ((TS_XP_PIN | TS_XN_PIN) << 2)) =
            ...
            //
            HWREG(ADC0_BASE + ADC_O_SSMUX3) = ADC_CTL_CH6;
