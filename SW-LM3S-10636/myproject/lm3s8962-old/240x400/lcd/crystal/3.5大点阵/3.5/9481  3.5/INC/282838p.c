//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
//  UG-2828GDEDF11 (2828-38p) Reference Code
//
//    Dot Matrix: 128RGB*128
...
#define SCLK	P3_5				// Serial Clock Input
#define SDIN	P3_6				// Serial Data Input
...
unsigned char code Ascii_2[108][5]={		// Refer to "Times New Roman" Font Database...
...
unsigned char i;
	...
	{
	// Show Pattern - Format
		Show_64k_Pattern(&Format,0x10,0x6F,0x20,0x5F);
		Delay(1);
	...
	// Scrolling (Partial or Full Screen)
		...
		Delay(1);
		Deactivate_Scroll();
		Horizontal_Scroll(0x00,0x01,0x38,0x48,0x01,0x03);
						// Rightward - Partial Screen
		Horizontal_Scroll(0x01,0x01,0x38,0x48,0x01,0x03);
	...
	// Show Pattern - Frame (Test Pattern)
		...
		Delay(1);
		Draw_Rectangle(0x10,0x6F,0x10,0x6F,0x01,0xF8,0x00);
		Delay(1);
		Draw_Rectangle(0x20,0x5F,0x20,0x5F,0x01,0x07,0xE0);
		Delay(1);
		Draw_Rectangle(0x30,0x4F,0x30,0x4F,0x01,0x00,0x1F);
		Delay(1);

	// Show String - Univision +886-37-586586
		Fill_Block(0x10,0x6F,0x10,0x6F,0x00,0x00);
		Show_String(1,&Name,0xFF,0xFF,0x25,0x34);
		Show_String(1,&Tel,0xFF,0xFF,0x16,0x44);
		Delay(2);
		Fill_RAM(0x00,0x00);		// Clear Screen
	}
}