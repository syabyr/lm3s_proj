/***************************************************************************
320x480 TTF crystal driver.
Manufature:ILITEK Ltd
Control:ILI9481
Author:mybays
Email:mybays@qq.com
Cortex-M3的液晶驱动
...
void LCD_320x480_ILI9481Init(void)
{
	...
	SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOH);		//D0-D7
	SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOJ);		//D8-D15
	...
	//数据高8bit
	GPIOPinTypeGPIOOutput(LCD_DATAH_BASE,0xff);	//0B0111 1111
	GPIOPinWrite(LCD_DATAH_BASE,0xff,0x00);

	//CS,RS,WR,RD信号,PE1,PE2,PE3,默认都为高电平
	GPIOPinTypeGPIOOutput(GPIO_PORTE_BASE,0x0f);	//0B0000 1111
	GPIOPinWrite(GPIO_PORTE_BASE,0x0E,0x0E);
	//RESET复位信号,PF4,默认为高电平
...
const tDisplay g_sLCD_320x480_9B95=
{
	sizeof(tDisplay),
	0,
#if defined(PORTRAIT) || defined (PORTRAIT_FLIP)
