...
main(void)
{
    uip_ipaddr_t ipaddr;			//ipv4下的原型为两个u16_t数据.
    static struct uip_eth_addr sTempAddr;		//为6个u8_t数据.
...
{
	u16_t ipaddr[2];
	uip_ipaddr(ipaddr,202,1,1,1);
	uip_connect(ipaddr,HTONS(10001));
		char *echo="connect me.\n";
		uip_send(echo,strlen(echo)+1);
		uip_send("mybays\n",7);
		uip_close();
}
