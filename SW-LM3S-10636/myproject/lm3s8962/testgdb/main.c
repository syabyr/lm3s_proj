...
if((i&1)!=1)
{
	...
	for (j=0;j<96;j++)
	RIT128x96x4ImageDraw(d,i,(int)j,2,1);
}
else
{
	temp[1]=(int)(adc0Value/11);
	if(temp[0]==temp[1])
		RIT128x96x4ImageDraw(a,i-1,93-temp[0],2,1);
	else
	{
		RIT128x96x4ImageDraw(b,i-1,93-temp[0],2,1);
		RIT128x96x4ImageDraw(c,i-1,93-temp[1],2,1);
	}
}
